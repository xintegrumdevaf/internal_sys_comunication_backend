...
> Este documento y los que lo acompañan (`01` a `05`, más `06_AI_PROMPTS.md` y `07_QUALITY_SUPERVISION.md`) son la **fuente única de verdad** para construir el sistema. No dependen de leer el código legacy. `ARCHITECTURE_CURRENT.md` y `MIGRATION_PLAN.md` quedan como contexto histórico de por qué se decidió reconstruir; no son requisitos de diseño.
...
- **IA (Ollama/Qwen u otro proveedor)**: se invoca directamente desde la API vía `AIProviderPort` (interpretación, composición de respuesta, OCR, transcripción, **análisis de calidad de atención humana**) — no a través de n8n.
...
flowchart TB
    ...
    subgraph API["API — fuente de verdad"]
        ...
        AI["AIProviderPort\n(interpretar, componer, OCR, audio, calidad)"]
        ARB[Case Arbitration]
        ENGINE[Workflow Engine]
        ESC[Escalation Service]
        AUTOM[Automation Gate]
        QA[Quality Supervision]
        DB[(PostgreSQL:\nnegocio + n8n_workflow_registry + quality)]
...
4. La IA se invoca **directamente desde la API** (`AIProviderPort`, adapter intercambiable — Ollama por defecto) para interpretar, componer respuestas, transcribir audio, leer comprobantes y **analizar calidad de atenciones humanas** (`07_QUALITY_SUPERVISION.md`) — n8n no participa en nada de esto.
...
| `06_AI_PROMPTS.md` | Prompts normativos del `AIProviderPort` (interpretación, composición, calidad) |
| `07_QUALITY_SUPERVISION.md` | Supervisión de calidad de atenciones humanas: triggers, scores, coaching híbrido, KPIs |
...
- Sin decisiones de negocio tomadas por el LLM: la IA entrega `intent/entities/confidence`, texto de respuesta sobre una plantilla ya decidida, o un `QualityAnalysis` tipado — la API decide qué ocurre, qué se dice y qué se hace con un hallazgo de calidad (nunca sanciones automáticas).
