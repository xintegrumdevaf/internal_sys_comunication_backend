...
Cómo trabajar
Sigue docs/spec/05_BUILD_PLAN.md una etapa a la vez, en orden. No adelantes trabajo de una etapa posterior aunque parezca conveniente.
Antes de codificar una etapa: resume en 3-5 líneas qué vas a construir y qué archivos vas a tocar/crear.
Cada etapa debe terminar con los tests de su "criterio de aceptación" (en 05_BUILD_PLAN.md) pasando en verde.
No avances a la siguiente etapa si la actual deja el build roto, migraciones pendientes, o tests fallando.
Un commit por etapa completada, mensaje descriptivo (feat(etapa-2): motor de workflow declarativo + SUPPORT_INTERNET).
